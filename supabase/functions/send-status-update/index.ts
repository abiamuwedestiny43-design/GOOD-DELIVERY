...
const generateStatusUpdateEmailHtml = (shipmentData: any, newStatus: string) => {
  const statusMessages = {
    'pending': 'Your shipment is being prepared for pickup.',
    'picked_up': 'Your shipment has been picked up and is on its way to our facility.',
  ...
  return `
    ...
    <head>
      <meta charset="utf-8">
      <title>Shipment Status Update - Sant Sport Logistics</title>
      <style>
        ...
        .status-box { background-color: ${statusColor[newStatus] || '#f59e0b'}20; border: 2px solid ${statusColor[newStatus] || '#f59e0b'}; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center; }
        .status-emoji { font-size: 48px; margin-bottom: 10px; }
        .status-title { font-size: 24px; font-weight: bold; color: ${statusColor[newStatus] || '#f59e0b'}; margin: 10px 0; }
      ...
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="detail-section">
            ...
            ${newStatus === 'delivered' ? 
              '<div class="detail-item">• Your package has been delivered successfully!</div><div class="detail-item">• Thank you for choosing Sant Sport Logistics</div>' :
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { to, shipmentData, newStatus }: StatusUpdateRequest = await req.json();
